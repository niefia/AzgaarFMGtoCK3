...
def runGen():
    ...
    print("Please make sure the Azgaar json is in the Output folder as input.json, and the Azgaar Cells Geojson is in the output folder as input.geojson")
    # Get output directory path from user input
    output_dir = input("Enter the full path to the output directory: ")
    ...
    spreadsheets.remove_emoji_from_json(os.path.join(output_dir, "input.json"),
                                        os.path.join(output_dir, "noemoji.json"))
    print("Emoji data removed from json")

    # Assign colors to Baronies in GeoJSON file
    spreadsheets.colorRandom(os.path.join(output_dir, "input.geojson"), os.path.join(output_dir, "output.geojson"))
