...
def simulate(img_in,iter_in,path_out,path_argv):
  ...
  img = Image.open(imginput).convert('L')
  img_as_np = np.asarray(img)
  
  
  # Grid dimension constants
  full_width = 200
  dim = img_as_np[0].size
  shape = [dim] * 2
  ...
  my_dir = os.path.dirname(path_argv)
  ...
  # `terrain` represents the actual terrain height we're interested 
  # img input remap
  terrain = np.true_divide(img_as_np, 255)
  ...
  file_not_present_sim = False
  count = 0
  output_path_sim = None
  while (file_not_present_sim == False):
    output_path_sim = os.path.join(path_out, 'simulation'+str(count)+'.npy')
    if (os.path.isfile(output_path_sim) == False):
      file_not_present_sim = True
    count += 1
    
  np.save(output_path_sim, util.normalize(terrain))
  #np.save(os.path.join(path_out, 'simulation'), util.normalize(terrain))
  make_grayscale_image.conv_grayscale(output_path_sim,path_out)
